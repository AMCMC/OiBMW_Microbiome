...
Most of the workflow is fixed and can be run as is. 
However, some options are left for the student to tune.
...
```{r load libraries, cache=T, message=FALSE, cache=T}
...
If we assume that most errors are random, we can reason that most errors results in unique, singleton sequences, while sequences that are replicated are more likely to be true variants. 

Dada2 requires a minimal of two observations of a sequence for it be even considered to be a true variant. How many unique sequences did we obtain? We use the `derepFastq` function from dada2 to dereplicate the forward and reverse reads.
...
Results show that overall substitution rates are even higher than what can be expected from the quality score.
...
```{r dada2 sequence inference, cache=T}
...
```{r dada2 asv merging, cache=T}
asv.m <- mergePairs(dadaF = asv.f, derepF = drf, dadaR = asv.r, derepR = drr, returnRejects = T, maxMismatch = 0)
head(asv.m)
```

This table shows the statistics for the top 10 most abundant asv pairs. Forward and reverse read mismatching is a common problem of Illumina sequencing. Therefore significant amount of reads are lost in this process.

How many ASVs do we have and how many reads do they represent?

```{r explore merging, cache=T}
length(asv.m$accept)
sum(asv.m$accept)
...
```{r compare ASV to reference, cache=T}
...
Most of these are actually stealthy chimeras which 
...
A phylogentic tree tries to capture the evolutionary relationships of various organims. We can use the sequence (dis)simmilarity of the ASVs to infer these phylogenetic relationsships. These trees help in generating context for our inferred sequences. 
...
Taxonomy is how we classify organisms in specific groups at certain ranks. While taxonomy tries to capture phylogeny (evolutionary relationships) at a course level, most of the classical labeling is based on phenotypic data which has resulted
...
With current day sequencing efforts we are better able to determine the phylogenetic relations which i
...
Some general insights into amplicon data characteristics.
